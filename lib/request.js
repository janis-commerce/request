...
module.exports = class Request {
	 ...
	 * @returns {Promise<RequestResponse>}
	 */
	static async call({ body, strictMode, safeMode, ...params }) {

		const response = await Request.makeRequest(params, body, strictMode);

		if(!safeMode && this.statusCode >= 400)
	...
	static requestCallback(res, resolve, reject, { strictMode, body, ...params }) {
		...
		res.on('end', () => {
			...
			const formattedResponse = new RequestResponse(incommingMessage, rawResponse);
