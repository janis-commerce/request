...
/**
	 * @typedef CallOptions
	 * @type {object}
	 * @property {object} headers - Custom headers on request. Define as { [headerName]: headerValue }
	 ...
	 * @property {boolean} strictMode - When this flag is set as true, the request response content-type should be application/json or error will thrown
 	 ...
 	 * @property {any} body - The request body (if request method accepts it). Can be a valid object, Array, string, or any serializable type.
	 */
...
/**
	 ...
	 * @property {object} headers Response headers. Formatted as { [headerName]: headerValue }
	 * @property {number} statusCode Response status code
	 * @property {string} statusMessage Response status message
	 * @property {any} body Response body. Can be a valid object, Array, string, or any serializable type.
		...
		return new Promise((resolve, reject) => {
			...
			const callback = res => {
				...
				res.on('end', () => {
					...
					if(strictMode && contentType.toLowerCase() !== defaultContentTypes[0]) {
						return reject(new RequestError('Response header mismatch with request body format',
					...
					return resolve(new RequestResponse(incommingMessage, responseBody, rawResponse));
				});
			...
			const nodeReq = protocols[requestProtocol].request(options, callback);
